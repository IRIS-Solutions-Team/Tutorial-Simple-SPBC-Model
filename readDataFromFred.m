...
%
% The only seasonal series is `CPILEGNS`. Run the standard `X13` on it, and
...
rawM.CPILEGNSsa = x13(rawM.CPILEGNS);
raw.CPILEGNSsa = x13(raw.CPILEGNS);

figure( );
plot(pct([rawM.CPILEGNS, rawM.CPILEGNSsa, rawM.CPILEGSL]));
axis tight;
grid on;
legend('CPILEGNS', 'CPILEGNS X13 adjusted', 'CPILEGSL');
...
raw.AHETPI = convert(raw.AHETPI, 4);
raw.CPILEGNS = convert(raw.CPILEGNS, 4);
raw.CPILEGNSsa = convert(raw.CPILEGNSsa, 4);
raw.CPILEGSL = convert(raw.CPILEGSL, 4);
raw.GS5 = convert(raw.GS5, 4);
raw.PCE = convert(raw.PCE, 4);
raw.PCEC96 = convert(raw.PCEC96, 4);
raw.TB3MS = convert(raw.TB3MS, 4);
raw.PCEP = raw.PCE/raw.PCEC96;

% raw = databank.apply(@(x) convert(x, Frequency.QUARTERLY), raw);
raw.SP500 = convert(raw.SP500, 4, 'IgnoreNaN=', true);
...
rawQ.SP500 = convert(rawD.SP500, 4, 'IgnoreNaN=', true);
...
dd = struct( );
dd.Infl = apct(raw.GDPCTPI); 
dd.Short = raw.TB3MS;
dd.Growth = apct(raw.GDPC1); 
dd.Wage = apct(raw.AHETPI);
dd.Asset = apct(raw.SP500);
dd.Long = raw.GS5;
...
save mat/read_data.mat d startHist endHist dd
